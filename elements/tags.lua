...
local tagStrings = {
	...
	['difficulty'] = [[function(u)
		if UnitCanAttack('player', u) then
			local l = UnitLevel(u) or UnitEffectiveLevel(u)
	...
	['level'] = [[function(u)
		local l = UnitLevel(u)
		if not (select(4, GetBuildInfo()) < 20000) and (UnitIsWildBattlePet(u) or UnitIsBattlePetCompanion(u)) then
	...
	if (not func) then
		...
		elseif (numTags == 2) then
			func = function(self)
				...
				_ENV._COLORS = parent.colors
				_ENV._FRAME = parent
				return self:SetFormattedText(format, args[1](unit, realUnit) or '', args[2](unit, realUnit) or '')
			end
